#create a tuple with different data types
tuplex = ("tuple", False, 3.2, 1)
print(tuplex)
print()

#create a numberic tuple
tuplex = (4, 6, 2, 8, 3, 1)
print(tuplex)
#tuple are immutable, so you can not add new elements
print()

#using merge of tuple with the + operation you can add a element and it will create a new truple
tuplex = tuplex + (9,)
print(tuplex)
print()

#count the number of occurrence of item 50 from a tuple
tuple1 = (50, 10, 60, 70, 50)
print(tuple1.count(50))
print()

#create a tuple
tuplex = (2, 4, 3, 5, 4, 6, 1)

#used tuple[start:stop]the start index is inclusive and the stop index is exclusive
_slice = tuplex[3:5]
print(_slice)
print()

#if the start index isn't defined, is taken from the beginning of the tuple
_slice = tuplex[:6]
print(_slice)
print()